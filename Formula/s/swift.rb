  ...
  def install
    ...
    if OS.mac?
      ...
      MacOS.sdk_locator.all_sdks.each do |sdk|
        build_sdk_interfaces_args = [
          "-sdk", sdk.path.to_s,
          "-o", "#{prefix}#{install_prefix}/lib/swift/macosx/prebuilt-modules",
          "-log-path", "#{logs}/build-sdk-interfaces",
          "-v"
        ]
        # Only build core modules since macOS 27 SDK to work around
        # "error: unable to resolve module dependency: 'MobileGestaltPrivate'"
        build_sdk_interfaces_args << "-core" if sdk.version >= 27

        system "#{prefix}#{install_prefix}/bin/swift", "build-sdk-interfaces", *build_sdk_interfaces_args
      end
